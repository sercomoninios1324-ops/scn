...
export default function Footer({ setCurrentPage, settings }: FooterProps) {
  ...
  const local = digits.startsWith('549')
    ...
    : digits.startsWith('54')
      ? digits.slice(2).replace(/^9/, '')
      : digits;
  const formattedWhatsapp =
    local.length === 10
      ? `+54 9 ${local.slice(0, 3)} ${local.slice(3, 6)}-${local.slice(6)}`
    ...
    <footer id="footer-main" className="border-t border-scn-border bg-scn-bg-card py-12 transition-colors">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-scn-text-normal">
              ...
              <li className="flex items-start space-x-2.5">
                ...
                <span>Venta por WhatsApp · Bahía Blanca</span>
              </li>
            </ul>
          </div>
        ...
        <div className="border-t border-scn-border mt-12 pt-6 flex flex-col sm:flex-row items-center justify-between text-xs text-scn-text-secondary">
          ...
          <p className="mt-2 sm:mt-0 inline-flex items-center gap-1.5">
            Diseñado con dedicación para Argentina
            <svg
              width="16"
              height="12"
              viewBox="0 0 16 12"
            ...
            >
              <rect width="16" height="4" y="0" fill="#74ACDF" />
              <rect width="16" height="4" y="4" fill="#FFFFFF" />
              <rect width="16" height="4" y="8" fill="#74ACDF" />
              <circle cx="8" cy="6" r="1.35" fill="#F6B40E" />
            </svg>
          </p>
        </div>
      </div>
