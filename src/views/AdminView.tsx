...
export default function AdminView({
  ...
  // Create Category modal/inline state
  ...
  // Fetch initial configs on tab switch or login
  useEffect(() => {
    if (siteSettings) {
  ...
  // Toggle quick actions: Is Active / Is Featured directly
  ...
  // Handle Drag & Drop Uploader (Converted to Base64 through simulated API)
  ...
  const handleRemoveImage = (index: number) => {
    setFormImages(prev => {
      ...
      // Re-assign cover if we removed the cover image
        ...
        <div className="rounded-2xl border border-scn-border bg-scn-bg-card p-8 shadow-md space-y-6">
          <div className="text-center space-y-2">
            ...
            <h1 className="font-display text-xl font-extrabold text-scn-text-title">
              Acceso SCN Administrador
            </h1>
            <p className="text-2xs text-scn-text-normal">
              Solo personal autorizado de SCN "Seamos como niños" tiene acceso aquí.
            </p>
          </div>
  ...
  // ----------------------------------------------------
  // RENDER: Form edit / create product screen
  // ----------------------------------------------------
  if (isEditing) {
    return (
      <div id="product-editor-container" className="page-enter mx-auto max-w-4xl px-4 py-8 space-y-6">
        ...
        <form id="product-crud-form" onSubmit={handleProductSave} className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          {/* Right Column details panel (Stock, SKU, features) */}
